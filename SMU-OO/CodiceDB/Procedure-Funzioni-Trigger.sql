...
BEGIN
 -- select controllo iban se esiste
  ...
  ELSE
      ...
      -- controllo che card ha associato il conto
      controllo_carta = controllo_tipocarta(iban_in);
      -- controlla se la cifra inserita sia maggiore di 0
      if soldi_in > 0 then
        
          if controllo_carta = 0 then --se è card di debito, devo controllare che il saldo del conto non superi 10000€
...
create function card(iban_in varchar) returns integer
as
$$
DECLARE
   controllo_tipocarta card.tipocarta%TYPE;
BEGIN
   select tipocarta
   ...
   cvv_generato := test.genera_cvv_casuale();
    ...
    account_email_in = lower(account_email_in);
       ...
       RAISE NOTICE E'---------------------------------------------------------------------------\nConto creato con successo! \n---------------------------------------------------------------------------';
        ...
        -- Inserisce anche in card i valori generati casuali
...
BEGIN
    ...
    --inserisce i dati in piggyBank
   INSERT INTO test.piggyBank(nomesalvadanaio, obiettivo, saldorisparmio, saldorimanente, descrizione, contocorrente_iban, datacreazione)
...
BEGIN
    ...
    -- controllo esistenza card inserita
  select count(pan)
  ...
  if controllo_esistenza > 0 then
      -- select dell'iban collegato alla card, per chiamare la funzione 'controllo_tipocarta'
        ...
        controllo_tipocarta = test.controllo_tipocarta(controllo_iban);
         if controllo_tipocarta = 1 then
              ...
              RAISE NOTICE E'---------------------------------------------------------------------------\nDowngrade effettuato! \n---------------------------------------------------------------------------';
               ...
               RAISE NOTICE E'---------------------------------------------------------------------------\nNon puoi effettuare il downgrade, la card è già di tipo Carta di Debito\n---------------------------------------------------------------------------';

         end if;
  else
      RAISE NOTICE E'---------------------------------------------------------------------------\nInserisci una card valida!\n---------------------------------------------------------------------------';
  end if;
    return 1;
END;
$$;
       ...
       if controllo_esistenza_ibanmitt > 0 and controllo_esistenza_ibandest > 0 then
           if iban_mittente_in = iban_destinatario_in then
              ...
              if controllo_saldo(soldi_In, iban_mittente_In) then
                ...
                -- funzione che controlla il tipo della card dell'iban mittente (0:debito, 1:credito)
                  if (controllo_tipocarta(iban_mittente_In)) then
                      if soldi_In < 3000 then
                          -- funzione che controlla il tipo della card dell'iban destinatario
                          if  (controllo_tipocarta(iban_destinatario_In) = 0) then
                              ...
                              where iban = iban_destinatario_In;
                                ...
                                --check se i soldi del conto con card di debito + i soldi in ingresso siano minori di 10000.
                                  if (controllo_saldo + soldi_In) < 10000 then
                                      --inserisco i dati in transaction
                                      INSERT INTO transaction(importo, causale, datatransazione, orariotransazione, iban2, tipologiapagamento, tipotransazione, categoriaentrata, categoriauscita, iban1)
                          ...
                          else -- caso card di credito del destinatario
                              --inserisco i dati in transaction
                              INSERT INTO transaction(importo, causale, datatransazione, orariotransazione, iban2, tipologiapagamento, tipotransazione, categoriaentrata, categoriauscita, iban1)
                                      VALUES (soldi_In, causale_In, current_date, current_time,  iban_mittente_In, 'Bonifico', 'Invia a', null, causale_In, iban_destinatario_In),
                                          (soldi_In, causale_In, current_date, current_time + interval '30 minutes', iban_destinatario_In, 'Bonifico', 'Riceve da', causale_In, null, iban_mittente_In);
                          ...
                          RAISE NOTICE E'---------------------------------------------------------------------------\nL\'importo supera il limite di soldi che è possibile inviare con una sola transaction.\n Se desideri puoi effettuare il downgrade!\n---------------------------------------------------------------------------';
                      end if;
                  else -- caso card di credito del mittente
                      -- controllo del tipo di card del destinatario
                      if (controllo_tipocarta(iban_destinatario_In) = 0) then
                      ...
                      else -- caso card di credito destinatario
                          -- inserisco i dati in transaction
                              INSERT INTO transaction(importo, causale, datatransazione, orariotransazione, iban2, tipologiapagamento, tipotransazione, categoriaentrata, categoriauscita, iban1)
                                      VALUES (soldi_In, causale_In, current_date, current_time,  iban_mittente_In, 'Bonifico', 'Invia a', null, causale_In, iban_destinatario_In),
                                          (soldi_In, causale_In, current_date, current_time + interval '30 minutes', iban_destinatario_In, 'Bonifico', 'Riceve da', causale_In, null, iban_mittente_In);
...
$$
              DECLARE
                  ...
                  controllo_obiettivo piggyBank.obiettivo%TYPE;
                  controllo_saldorimanente piggyBank.saldorimanente%TYPE;
                  controllo_saldorisparmio piggyBank.saldorisparmio%TYPE;
              BEGIN
                ...
                -- controlla se esiste l'iban inserito
                  ...
                  if controllo_iban > 0 then
                      -- controlla se esiste il piggyBank inserito
                      ...
                      if controllo_salvadanaio > 0 then
                        ...
                        if soldi_in > 0 then
                          if controllo_saldo >= soldi_In then
                            ...
                            -- select che prende l'obiettivo del piggyBank
                              select obiettivo
                              into controllo_obiettivo
                              from test.piggyBank
                              where nomesalvadanaio ILIKE nome_In and contocorrente_iban = iban_In;

                            -- aggiorna il saldo risparmio del piggyBank
                              UPDATE test.piggyBank
                              SET saldorisparmio = saldorisparmio + soldi_In
                              WHERE contocorrente_iban = iban_In AND nomesalvadanaio ILIKE nome_In;

                            -- aggiorna il saldo rimanente del piggyBank
                              UPDATE test.piggyBank
                              SET saldorimanente = piggyBank.obiettivo - piggyBank.saldorisparmio
                              WHERE contocorrente_iban = iban_In AND nomesalvadanaio ILIKE nome_In;
                          ...
                          RAISE NOTICE E'---------------------------------------------------------------------------\nNon esiste nessun piggyBank con questo nome!\n---------------------------------------------------------------------------';
                      end if;
...
BEGIN
   ...
   if controllo_iban > 0 then
       -- controllo se esiste il piggyBank inserito
       select count(idsalvadanaio)
       into controllo_salvadanaio
       from test.piggyBank
       where contocorrente_iban = iban_In AND nomesalvadanaio ILIKE nome_In;
        ...
        -- controllo se il saldo risparmio è sufficiente
            if soldi_in > 0 then
               if controllo_saldorisparmio >= soldi_In then
                ...
                -- select dell'obiettivo del piggyBank
                   select obiettivo
                   into controllo_obiettivo
                   from test.piggyBank
                   ...
                   where nomesalvadanaio ILIKE nome_In and contocorrente_iban = iban_In;
                    ...
                    -- controllo se il piggyBank ha raggiunto l'obiettivo
                   if  controllo_saldorisparmio >= controllo_obiettivo then
                       RAISE NOTICE E'---------------------------------------------------------------------------\n Saldo piggyBank attuale: %.\n---------------------------------------------------------------------------', controllo_saldorisparmio;
...
BEGIN
   ...
   if controllo_iban > 0 then
       --select controllo esistenza piggyBank inserito
       select count(idsalvadanaio)
       into controllo_salvadanaio
       from test.piggyBank
       where contocorrente_iban = iban_In AND nomesalvadanaio ILIKE nome_In;
       ...
       if controllo_salvadanaio > 0 then
           --select saldo risparmio piggyBank
           select saldorisparmio
           into controllo_saldorisparmio
           from test.piggyBank
           where nomesalvadanaio ILIKE nome_In and contocorrente_iban = iban_In;
           --se il saldo è = 0 allora elimina il piggyBank, altrimenti no
...
BEGIN
   ...
   if controllo_iban > 0 and controllo_data != 0 then
    ...
    RAISE NOTICE E'---------------------------------------------------------------------------\nNon esiste nessun conto con questo Iban!\n---------------------------------------------------------------------------';
   elseif controllo_iban > 0 and controllo_data = 0 then
       RAISE NOTICE E'---------------------------------------------------------------------------\nNessuna transaction trovata per questo periodo!\n---------------------------------------------------------------------------';
   else
...
BEGIN
  ...
  if controllo_esistenza > 0 then
      ...
      if controllo_tipocarta = 0 then
          ...
          RAISE NOTICE E'---------------------------------------------------------------------------\nNon puoi effettuare l\'upgrade perchè la card è già di tipo Carta di Credito \n---------------------------------------------------------------------------';
    ...
    if controllo_cod > 0 then
       ...
       -- Mi permette di visualizzare il conto delle persone registrate
        ...
        FROM
            ContoCorrente cc
        JOIN
            card c ON Cc.Iban = C.Contocorrente_Iban
        JOIN
...
begin
    ...
    if controllo_iban > 0 then
        if controllo_raccolta > 0 then
            ...
            from colleziona c join transaction t
...
BEGIN
    ...
    if controllo_iban > 0 then
        --query che ritorna i dati del piggyBank inserito
        RETURN QUERY
        ...
        RAISE NOTICE E'---------------------------------------------------------------------------\nQuesto conto non ha nessun piggyBank creato!\n---------------------------------------------------------------------------';
    end if;
...
$$
DECLARE
   ultima_transazione int; --transaction.idtransazione%TYPE;
...
BEGIN
   ...
   if controllo_iban > 0 then
       -- select dell'ultima transaction dell'iban inserito
       ...
       -- select dell'ultima notifica collegata all'ultima transaction
